""" 
Write a function, binary_search_tree_includes, that takes in the root of a binary search tree 
...
def binary_search_tree_includes(node, target):
    if node is None:
        return False  
    ...
    if target < node.val:
        return binary_search_tree_includes(node.left, target)
    else:
        return binary_search_tree_includes(node.right, target)
